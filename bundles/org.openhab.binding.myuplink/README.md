...
This binding should in general be compatible with heat pump models that support MyUplink.
In general read access is supported for all channels.
Write access is only supported for a small subset of channels.
Write access will only be available with a paid subscription for "manage" at MyUplink.
...
Auto-Discovery is not supported, as credentials are necessary to login into MyUplink.
...
|-------------------|-------------------|-------------------------------------------------------|
| VVM310 / 500      | vvm310            | reduced set of channels based on MyUplink website   |
| VVM320 / 325      | vvm320            | reduced set of channels based on MyUplink website   |
| F730              | f730              | reduced set of channels based on MyUplink website   |
| F750              | f750              | reduced set of channels based on MyUplink website   |
| F1145 / 1245      | f1145             | reduced set of channels based on MyUplink website   |
| F1155 / 1255      | f1155             | reduced set of channels based on MyUplink website   |

The following configuration parameters are available for this thing:

- **user** (required)  
username used to login on MyUplink

- **password** (required)  
password used to login on MyUplink

- **nibeId** (required)  
Id of your heatpump in MyUplink (can be found in the URL after successful login: https://www.myuplink.com/System/**<nibeId>>**/Status/Overview)

- **pollingInterval**  
interval (seconds) in which values are retrieved from MyUplink.
Setting less than 60 seconds does not make any sense as the heat pump only provides periodic updates to MyUplink.
(default = 60)
...
```
myuplink:vvm320:mynibe [ user="...", password="...", nibeId="..."]
```

- with pollingInterval

```
myuplink:vvm320:mynibe[ user="...", password="...", nibeId="...", pollingInterval=... ]
```

- multiple heat pumps

```
myuplink:vvm320:home1 [ user="...", password="...", nibeId="..."]
myuplink:vvm320:home2  [ user="...", password="...", nibeId="..."]
```
...
```
myuplink:vvm320:mynibe     [ user="nibe@my-domain.de", password="secret123", nibeId="4711", pollingInterval=300, customChannel01=47376, customChannel02=48009 ]
```
...
```
Number:Temperature      NIBE_SUPPLY            "Vorlauf"                         { channel="myuplink:vvm320:mynibe:base#40008" }
Number:Temperature      NIBE_RETURN            "Rücklauf [%.2f °F]"              { channel="myuplink:vvm320:mynibe:base#40012" }
Number:Temperature      NIBE_HW_TOP            "Brauchwasser oben"               { channel="myuplink:vvm320:mynibe:hotwater#40013" }
Number:Energy           NIBE_HM_HEAT           "WM Heizung"                      { channel="myuplink:vvm320:mynibe:base#44308" }
Switch                  NIBE_COMP_DEFROST      "Enteisung"                       { channel="myuplink:vvm320:mynibe:compressor#44703" }
Number                  NIBE_HW_MODE           "Modus [MAP(hwmode.map):%s]"      { channel="myuplink:vvm320:mynibe:hotwater#47041" }

Number                  NIBE_CUSTOM_01         "Custom 01"                       { channel="myuplink:vvm320:mynibe:custom#CH01" }
```
